...
const CustomerReviewsSliders = () => {
  let { CurrentStocks, AccessToken } = useLayout();

  const { data, isLoading } = useQuery({
    queryKey: ["CustomerReviewsSliders", { CurrentStocks, AccessToken }],
    queryFn: async () => {
  ...
  return (
    <div className="px-3 pt-3.5">
      <div className="flex justify-between">
        ...
        <Link to="/reviews" className="text-gold text-sm">
          Xem thêm
        </Link>
      </div>
      <div>
        {isLoading && (
          <div>
            <div className="bg-white border rounded-md p-3 mb-3 last:mb-0">
            ...
            {data &&
              data.map((item, index) => (
                <SwiperSlide className="w-[90%]" key={index}>
                  <NavLink to={`/reviews/${item.id}`} className="bg-white border rounded-md p-3 block shadow">
                    <div className="flex items-center mb-3 gap-3">
                      <div className="w-16">
                        <img
