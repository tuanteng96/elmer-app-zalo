...
const NewsPage = () => {
  ...
  const Cates = useQuery({
    queryKey: ["NewsCate"],
    queryFn: async () => {
      let rs = await NewsAPI.getInfoToCateID("11361");
  ...
  const NewsOutstanding = useQuery({
    queryKey: ["NewsOutstanding"],
    queryFn: async () => {
      const { data } = await NewsAPI.getListToID("11363");
  ...
  return (
    <Page className="page !overflow-hidden flex flex-col" hideScrollbar>
          ...
          <div className="h-full overflow-auto no-scrollbar">
            <div className="px-3 py-3.5 bg-app">
              <div className="mb-3.5 font-semibold text-white flex justify-between items-center uppercase">
                {NewsOutstanding.isLoading ? (
                  <div className="h-4 bg-gray-200 rounded-full w-2/5"></div>
                ) : (
                  <>
                    {
                      NewsOutstanding.data && NewsOutstanding.data.length > 0 ? <>{NewsOutstanding.data[0].source.CateTitle}</> : <></>
                    }
                  </>
                )}
              ...
              <div className="border-white border-[5px] rounded-md bg-white">
                ...
                {!NewsOutstanding.isLoading && (
                  <Swiper
                    ...
                    slidesPerView="auto"
                  >
                    {NewsOutstanding.data &&
                      NewsOutstanding.data.slice(0, 3).map((item, index) => (
