...
const CooperatePage = () => {
  ...
  const Cates = useQuery({
    queryKey: ["CooperateCate"],
    queryFn: async () => {
      let rs = await NewsAPI.getInfoToCateID("11378");
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["CooperateList", filters],
    queryFn: async () => {
  ...
  const CooperateOutstanding = useQuery({
    queryKey: ["CooperateOutstanding"],
    queryFn: async () => {
      const { data } = await NewsAPI.getListToID("11379");
        ...
        <div className="h-full overflow-auto no-scrollbar pb-safe-bottom">
          <div className="px-3 py-3.5 bg-app">
            <div className="mb-3.5 font-semibold text-white flex justify-between items-center uppercase">
              {CooperateOutstanding.isLoading ? (
                <div className="h-4 bg-gray-200 rounded-full w-2/5"></div>
              ) : (
                <>
                  {
                    CooperateOutstanding.data && CooperateOutstanding.data.length > 0 ? <>{CooperateOutstanding.data[0].source.CateTitle}</> : <></>
                  }
                </>
              )}
            ...
            <div className="border-white border-[5px] rounded-md bg-white">
              ...
              {!CooperateOutstanding.isLoading && (
                <Swiper
                  ...
                  pagination={{
                    el: "#containerForBulletsCooperate",
                    type: "bullets",
                  ...
                  slidesPerView="auto"
                >
                  {CooperateOutstanding.data &&
                    CooperateOutstanding.data.map((item, index) => (
            ...
            <div className="flex justify-center mt-3.5">
              <div
                ...
                id="containerForBulletsCooperate"
              ></div>
            </div>
          </div>
            ...
            {!isLoading && (
              <>
                {data && data.map((item, index) => (
                  <NavLink
                    to={"/courses/" + item.id}
                    state={{ dataProps: item, prevState: state?.prevState || null }}
                    className="bg-white shadow-3xl border-[5px] border-white flex gap-2 mb-3 rounded overflow-hidden last:mb-0"
                    key={index}
                  >
                    <div className="w-2/4">
